    ...
    await Conversation.updateOne(
      { _id: keeperId },
      {
        $set: {
          ...
          contactPhoneDigits:
            mergedConversation.contactPhoneDigits || keeper.contactPhoneDigits || '',
...
const run = async () => {
  const batchSize = parsePositiveInt(
    readArg(['--batch-size', '--batchSize']) || process.env.CONVERSATION_DEDUPE_BATCH_SIZE,
    500
  );
  ...
  const cursor = Conversation.find(filters)
    .select(
      '_id userId companyId contactId contactPhone contactPhoneDigits contactName status assignedTo assignedToId tags priority lastMessageTime lastMessage lastMessageMediaType lastMessageAttachmentName lastMessageAttachmentPages lastMessageFrom lastMessageWhatsappMessageId lastMessageStatus unreadCount notes resolvedAt createdAt updatedAt'
    )
  ...
  console.log(
    JSON.stringify(
      {
        event: 'conversation_dedupe_complete',
        dryRun,
...
run().catch(async (error) => {
  console.error('Conversation dedupe cleanup failed:', error);
  try {
    await mongoose.connection.close();
  } catch {
